...
module.exports = {
  meta: {
    ...
    schema: [{
      ...
      properties: {
        ...
        // Whether alphabetical sorting should be enforced for callbacks
        noSortNonCallbackAlphabetically: {
          type: 'boolean'
        },
  ...
  create(context) {
    ...
    const noSortNonCallbackAlphabetically = configuration.noSortNonCallbackAlphabetically || false;
    ...
    function sorter(a, b) {
      ...
      const aIsCallback = isCallbackPropName(aKey);
      const bIsCallback = isCallbackPropName(bKey);
      const sortingCallbacks = aIsCallback && bIsCallback;
      if (callbacksLast) {
      ...
      if (noSortAlphabetically || (noSortNonCallbackAlphabetically && !sortingCallbacks)) {
        return 0;
      }
    ...
    function checkSorted(declarations) {
      ...
      declarations.reduce((prev, curr, idx, decls) => {
        ...
        const sortingCallbacks = previousIsCallback && currentIsCallback;
        ...
        if (
          (!noSortAlphabetically && (!noSortNonCallbackAlphabetically || sortingCallbacks)) &&
          currentPropName < prevPropName
        ) {
          context.report({
            node: curr,
