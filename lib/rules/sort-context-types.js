    ...
    return {
      CallExpression(node) {
        ...
        if (firstArg.properties) {
          checkSorted(firstArg.properties, {sortingContext: false});
        } else if (firstArg.type === 'Identifier') {
          const variable = variableUtil.findVariableByName(context, firstArg.name);
          if (variable && variable.properties) {
            checkSorted(variable.properties, {sortingContext: false});
          }
        }
      },

      ClassProperty(node) {
        if (propsUtil.isContextTypesDeclaration(node)) {
      ...
      ObjectExpression(node) {
        node.properties.forEach((property) => {
          ...
          if (property.value.type === 'ObjectExpression') {
            if (propsUtil.isContextTypesDeclaration(property)) {
